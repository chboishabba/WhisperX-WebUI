...
- `main.py` wires up the FastAPI app and includes the routers.
- `routers/` defines the API endpoints (task submission, status polling, file outputs).
- `configs/` stores `config.yaml` and `.env` for runtime settings.
- `db/` and `records.db` hold task metadata and state.
- `cache/` stores files generated by API tasks (BGM separation outputs, transcripts, exports).
- `tests/` contains API test coverage.
...
2. Edit `nginx/nginx.conf` for your domain name.
...
## Configuration
You can set some server configurations in `configs/config.yaml`.
